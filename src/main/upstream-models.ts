...
export type FetchUpstreamModelsResult =
  | { ok: true; modelIds: string[]; modelGroups?: ModelProviderModelGroup[]; modelDetails?: ModelDetail[]; latencyMs?: number }
...
export async function fetchUpstreamModelIds(
  ...
  try {
    ...
    const modelDetails: ModelDetail[] = [...ids].map((id) => {
      const row = (data as { data?: Array<{ id?: string; context_length?: number }> }).data?.find(
        (r) => r && typeof r === 'object' && (r as { id?: unknown }).id === id
      )
      return {
        id,
