...
export class AppErrorBoundary extends Component<Props, State> {
  ...
  private handleReport = async (): Promise<void> => {
    ...
    let appVersion = ''
    try {
      if (typeof window !== 'undefined' && window.dsGui?.getAppVersion) {
        appVersion = await window.dsGui.getAppVersion()
      }
    } catch { /* ignore */ }
    const platformInfo = typeof navigator !== 'undefined' ? navigator.platform : ''
    const enrichedContext: ErrorContext | null = errorContext
      ? { ...errorContext, appVersion, platformInfo }
      : { category: 'unknown', message: error?.message ?? '', appVersion, platformInfo, capturedAt: Date.now() }
    const reportBody = getErrorReportBody(error, enrichedContext)
    const categoryKey = enrichedContext?.category ? getFriendlyErrorKey(enrichedContext.category) : ''
  ...
  override render(): ReactNode {
    ...
    const canRetry = errorContext?.retryable === true && this.retryCount < this.maxRetries
