...
import { BarChart3, MessageSquare, Zap, Clock, TrendingUp, Activity, Download, Trash2 } from 'lucide-react'
import { getUsageStats, formatNumber, getTopFeatures, getDailyUsage, getWeeklyUsage, exportUsageStats, clearUsageStats } from '../lib/usage-stats'
...
export function UsageStatsView(): ReactElement {
  ...
  const handleClear = () => {
    if (window.confirm(t('usageStatsClearConfirm') || 'Are you sure you want to clear all usage data?')) {
      clearUsageStats()
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg border border-ds-border bg-ds-card px-3 py-2 text-sm font-medium text-ds-ink transition hover:bg-ds-hover"
        >
          <Download className="h-4 w-4" />
          {t('usageStatsExport') || 'Export Data'}
        </button>
        <button
          type="button"
          onClick={handleClear}
          className="inline-flex items-center gap-1.5 rounded-lg border border-rose-500/30 bg-rose-500/5 px-3 py-2 text-sm font-medium text-rose-600 dark:text-rose-400 transition hover:bg-rose-500/10"
        >
          <Trash2 className="h-4 w-4" />
          {t('usageStatsClear') || 'Clear Data'}
        </button>
      </div>
    </div>
  )
