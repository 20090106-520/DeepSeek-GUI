...
export function ModelMarketView(): ReactElement {
  ...
  const handleTestProvider = useCallback(async (providerId: string) => {
    ...
    try {
      ...
      const latencyMs = response.ok && response.latencyMs ? response.latencyMs : Date.now() - start
      if (response.ok) {
        ...
        try {
          const currentSettings = await window.dsGui.getSettings()
          const existingProvider = currentSettings.provider?.providers?.find((p) => p.id === providerId)
          if (!existingProvider) {
            const preset = PRESET_PROVIDERS[providerId as PresetProviderId]
            if (preset) {
