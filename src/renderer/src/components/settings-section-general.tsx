...
export function GeneralSettingsSection({ ctx }: { ctx: Record<string, any> }): ReactElement {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const latency = response.ok && response.latencyMs ? response.latencyMs : null
                    ...
                    <div className="flex flex-col gap-2 md:max-w-md">
                      ...
                      {connectionStatus !== 'idle' && connectionMessage && (
                        <div className={`flex items-center gap-2 text-[13px] ${connectionStatus === 'success' ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
                          ...
                          {connectionMessage}
                          {connectionLatency != null && connectionStatus === 'success' && (
                            <span className="text-[11px] text-ds-muted">({connectionLatency}ms)</span>
                          )}
                        </div>
                      )}
                    </div>
                ...
                <SettingRow
                  ...
                  control={
                    ...
                    >
                      ...
                      <option value="siliconflow">{PRESET_PROVIDERS.siliconflow.name}</option>
                      <option value="doubao">{PRESET_PROVIDERS.doubao.name}</option>
                      <option value="qwen">{PRESET_PROVIDERS.qwen.name}</option>
                      <option value="ernie">{PRESET_PROVIDERS.ernie.name}</option>
                ...
                <SettingRow
                  ...
                  control={
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-ds-muted ml-1">{form.uiZoom ?? 100}%</span>
                    </div>
                  }
                />
                <SettingRow
